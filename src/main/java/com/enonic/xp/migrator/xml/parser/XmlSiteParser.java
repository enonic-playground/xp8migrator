    ...
    extends XmlModelParser<XmlSiteParser>
{
    ...
    private List<ResponseProcessorDescriptor> parseProcessorDescriptors( final DomElement processorDescriptorsParent )
    {
        ...
        {
            return processorDescriptorsParent.getChildren( PROCESSOR_DESCRIPTOR_TAG_NAME ).stream().map(
                this::toProcessorDescriptor ).collect( Collectors.toList() );
        }
    ...
    private List<ControllerMappingDescriptor> parseMappingDescriptors( final DomElement mappingDescriptorsParent )
    {
        ...
        {
            return mappingDescriptorsParent.getChildren( MAPPING_DESCRIPTOR_TAG_NAME ).stream().map( this::toMappingDescriptor ).collect(
                Collectors.toList() );
        }
        return Collections.emptyList();
    }
