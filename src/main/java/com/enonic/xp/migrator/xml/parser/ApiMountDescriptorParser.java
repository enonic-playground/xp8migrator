...
public class ApiMountDescriptorParser
{
    ...
    public DescriptorKeys parse()
    {
        if ( apisElement != null )
        {
            return apisElement.getChildren( API_DESCRIPTOR_TAG_NAME ).stream().map( this::toDescriptorKey ).collect(
                DescriptorKeys.collector() );
        }
