    ...
    extends XmlModelParser<XmlApiDescriptorParser>
{
        ...
        throws Exception
    {
        ...
        {
            PrincipalKeys allowedPrincipalKeys = allowedPrincipals.getChildren( PRINCIPAL_TAG_NAME ).stream().map(
                allowedPrincipal -> PrincipalKey.from( allowedPrincipal.getValue().trim() ) ).collect( PrincipalKeys.collector() );
